...
def confirmOTPFunder(request):
    ...
    if request.method == "POST": 
        emailAddress = request.session['username']
        print(emailAddress)
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            if funder.isConfirmed:
                ...
                print(productList)
                print(funder.maximum_funding_capacity,funder.minimum_accepted_score)
                context['productList']=[]
                for i in productList:
                    if i.company_ceo.budgetRange < funder.maximum_funding_capacity and i.predicted_score >= funder.minimum_accepted_score:
                        context['productList'].append(i)
                      # add data in productList.
                #print(context['productList'][0].productType)
                return render(request, 'Funder/funderDashBoard.html', context) # responce
            else:
                context['userData'] = funder
                return render(request, 'Funder/funderNotConfirmed.html', context) # responce. 
